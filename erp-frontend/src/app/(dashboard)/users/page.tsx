...
// 1. باز کردن مودال
  const handleSpecialPermissions = async (user: User) => {
    ...
    try {
      ...
      // دریافت جزئیات یوزر
      const { data } = await apiClient.get<any>(`/Permissions/user-detail/${user.id}`);
      
      const rIds: number[] = data.rolePermissionIds;
      const overrides: { permissionId: number, isGranted: boolean }[] = data.userOverrides;
...
const handleCopyPermissions = async () => {
    ...
    if (!confirm("آیا مطمئن هستید؟ تمام دسترسی‌های فعلی این کاربر حذف و با دسترسی‌های کاربر انتخابی جایگزین می‌شود.")) return;
                    ...
                    <tr key={user.id} className="border-b hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 flex gap-3">
                        ...
                        <PermissionGuard permission="UserAccess.SpecialPermissions">
                            ...
                            >
                                <Key size={18} /> {/* آیکون کلید را از lucide ایمپورت کنید */}
                            </button>
                        </PermissionGuard>
  ...
  {/* مودال هوشمند */}
        ...
        <Modal
          ...
          <div className="mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
              <label className="block text-xs font-bold text-gray-700 mb-2">کپی دسترسی از کاربر دیگر:</label>
          ...
          <hr className="my-4" />
            ...
            <div className="space-y-4">
                <div className="bg-yellow-50 p-3 rounded text-xs text-yellow-800">
                    نکته: تیک‌های <span className="font-bold text-green-700">سبز</span> دسترسی اضافه، و موارد <span className="font-bold text-red-600 line-through">قرمز</span> محرومیت هستند.
                </div>
                ...
                <div className="flex justify-end gap-2 pt-4 border-t">
                    <button onClick={() => setIsPermModalOpen(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg">انصراف</button>
                    <button onClick={saveUserPermissions} className="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">ذخیره</button>
                </div>
            </div>
        </Modal>
