...
export default function EditUnitForm({ unit, onSuccess, onCancel }: Props) {
  ...
  useEffect(() => {
    apiClient
      ...
      .then((res) => setUnitsList(res.data));
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...formData,
        baseUnitId:
          formData.baseUnitId === "" ? null : Number(formData.baseUnitId),
      };
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setFormData({ ...formData, precision: Number(e.target.value) })
            }
            className="w-full rounded-lg border border-gray-300 p-2 text-sm outline-none focus:border-blue-500"
          />
        </div>
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setFormData({ ...formData, baseUnitId: e.target.value })
            }
          ...
          >
            ...
            {unitsList
              ...
              .map(
                (
                  u // خود واحد را از لیست حذف می‌کنیم تا سیکل نشود
                ) => (
      ...
      {formData.baseUnitId !== "" && (
        <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>
              {
                unitsList.find((u) => u.id == Number(formData.baseUnitId))
                  ?.title
              }
            </span>
          </div>
